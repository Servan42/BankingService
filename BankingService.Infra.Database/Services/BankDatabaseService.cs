        ...
        public void UpdateTransactions(List<UpdatableTransactionDto> transactionsDto)
        {
            ...
            foreach(var transactionToUpdate in ResolveUpdatebleTransactionDtoCategoryId(transactionsDto))
            {
                if (!transactionToUpdate.Id.HasValue)
                    throw new InvalidOperationException($"Transaction '{transactionToUpdate.GetUniqueIdentifier()}' cannot be updated because it does not have an Id. Aborting");

                if (!storedTransactions.Data.ContainsKey(transactionToUpdate.Id.Value))
                    throw new InvalidOperationException($"Transaction '{transactionToUpdate.Id.Value}' cannot be updated because it is not present in database. Aborting.");
